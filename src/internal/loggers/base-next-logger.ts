...
import { DEFAULT_LOGGING_TRANSPORTS, DEFAULT_PRETTY_LOGGING } from './constants';
...
const includeErrorContext = (parsed: ParsedArgs, error: types.LoggerError): ParsedArgs => {
  ...
  throw new UnreachableCaseError(`Invalid error object provided to logger: ${String(error)}.`);
};
...
export abstract class BaseNextLogger extends AbstractLogger {
  ...
  constructor(
    ...
    super(name, { environment: loggerEnvironment, globalContext, level, vercelEnvironment });
    this._pretty = pretty;
  ...
  protected static createConfig(
    ...
    return {
      ...
      transports: {
        [EnvironmentNames.LOCAL]: ['browser'],
        [EnvironmentNames.PREVIEW]: ['browser'],
  ...
  private parseErrorArgs(
    arg0: string | types.CaptureContext | types.LoggerError,
    ...
    if (types.isLoggerError(arg0) && typeof arg1 === 'string') {
      ...
      return includeErrorContext({ context, message: arg1, options }, arg0);
      ...
      const [options, context] = parseCaptureContext(arg1 ?? {});
      return { context, message: arg0, options };
    }
    const [options, context] = parseCaptureContext(arg1 ?? {});
  ...
  /**
   * Logs a debug message, {@link string}, set of contextual data, {@link Record<string, unknown>},
   * or a combination of both, with a log level "debug", via the {@link NextLogger} instance.
   */
  public debug(message: Record<string, unknown> | string, context?: Record<string, unknown>): void {
  ...
  public error(data: string | types.LoggerError, context?: types.CaptureContext): void;
   ...
   *   and additional context that should be logged with the message (in the case that the first
   ...
   * @example
   * ```ts
   * // Dispatches error and message to Sentry.  The User's ID will be included in the log context.
   ...
   */
  public error(
    data: string | types.CaptureContext | types.LoggerError,
    msg?: string | types.CaptureContext,
    ctx?: types.CaptureContext,
  ): void {
    const { context, error, message, options } = this.parseErrorArgs(data, msg, ctx);
    ...
    if (options.capture) {
      if (error && message) {
        this.captureError(error, { ...context, level: LogLevels.ERROR, message });
      } else if (error) {
   ...
   */
  public info(message: Record<string, unknown> | string, context?: Record<string, unknown>): void {
    if (typeof message === 'string') {
  ...
  public warn(data: string | types.LoggerError, context?: types.CaptureContext): void;
  ...
  public warn(
    ...
    if (options.capture) {
      if (error && message) {
        this.captureError(error, { ...context, level: LogLevels.WARN, message });
      } else if (error) {
        this.captureError(error, { ...context, level: LogLevels.WARN, message });
      } else if (message) {
        this.captureMessage(message, { ...context, level: LogLevels.WARN, message });
      }
    }
  }
  ...
  protected get config() {
    ...
    if (this.hasTransport('browser')) {
      return { ...base, browser: { asObject: true, write: this.browserWriter } };
    }
