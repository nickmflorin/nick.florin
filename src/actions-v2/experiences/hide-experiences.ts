...
export const hideExperiences = async (
  _ids: string[],
  ...
  if (invalidIds.length !== 0) {
    ...
    logger.error(`Encountered invalid experience ID(s) when hiding experiences: ${humanized}.`, {
      ids,
      invalidIds,
    });
    const err = ApiClientGlobalError.BadRequest({
  ...
  if (experiences.some(exp => !exp.visible)) {
    const humanized = humanizeList(
      experiences.filter(exp => !exp.visible).map(exp => exp.id),
      { conjunction: "and", formatter: v => `'${v}'` },
    );
    logger.warn(
      `A request to hide experiences included experience ID(s) ${humanized} associated ` +
        "with experiences that are already hidden.",
      { ids: experiences.filter(exp => !exp.visible).map(exp => exp.id) },
    );
  }
