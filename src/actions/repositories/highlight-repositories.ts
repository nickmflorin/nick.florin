...
export const highlightRepositories = async (
  _ids: string[],
  ...
  if (invalidIds.length !== 0) {
    ...
    logger.error(
      `Encountered invalid repository ID(s) when highlighting repositories: ${humanized}.`,
      {
        ids,
        invalidIds,
      },
  ...
  if (repositories.some(proj => proj.highlighted)) {
    ...
    logger.warn(
      `A request to highlight repositories contained repository ID(s) ${humanized} ` +
        "associated with repositories that are already highlighted.",
