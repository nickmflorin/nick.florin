...
export const hideRepositories = async (
  _ids: string[],
  ...
  if (invalidIds.length !== 0) {
    ...
    logger.error(`Encountered invalid repository ID(s) when hiding repositories: ${humanized}.`, {
      ids,
      invalidIds,
    });
    const err = ApiClientGlobalError.BadRequest({
  ...
  if (repositories.some(proj => !proj.visible)) {
    ...
    logger.warn(
      `A request to hide repositories included repository ID(s) ${humanized} associated ` +
        "with repositories that are already hidden.",
