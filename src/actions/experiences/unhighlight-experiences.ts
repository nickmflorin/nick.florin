...
export const unhighlightExperiences = async (
  _ids: string[],
  ...
  if (invalidIds.length !== 0) {
    ...
    logger.error(
      `Encountered invalid experience ID(s) when unhighlighting experiences: ${humanized}.`,
      { ids, invalidIds },
    );
    const err = ApiClientGlobalError.BadRequest({
  ...
  if (experiences.some(exp => !exp.highlighted)) {
    ...
    logger.warn(
      `A request to unhighlight experiences contained experience ID(s) ${humanized} associated with ` +
        "experiences that are already unhighlighted.",
