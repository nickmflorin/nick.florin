...
export const prioritizeSkills = async (
  _ids: string[],
  ...
  if (invalidIds.length !== 0) {
    ...
    logger.error(`Encountered invalid skill ID(s) when prioritizing skills: ${humanized}.`, {
      ids,
      invalidIds,
    });
    const err = ApiClientGlobalError.BadRequest({
  ...
  if (skills.some(sk => sk.prioritized)) {
    ...
    logger.warn(
      `A request to prioritize skills contained skill ID(s) ${humanized} associated with skills that ` +
        "are already prioritized.",
  ...
  await db.skill.updateMany({
    where: { id: { in: ids } },
    data: { prioritized: true, updatedById: user.id },
  });
