...
const isSearchParamsLike = (params: unknown): params is URLSearchParams =>
...
export const searchParamsIterator = function* <Q extends QueryParams>(
  ...
  if (params instanceof Map) {
    ...
    /* We want to also account for other forms of URLSearchParams, like NextJS's
       ReadOnlyURLSearchParams.  This isn't iron-clad fool-proof, but for now it will suffice. */
  } else if (isSearchParamsLike(params)) {
...
const initialQueryParamState: {
  [key in QueryParamsForm]: () => QueryParams<key, string>;
} = {
...
export const getInitialQueryParamsState = <O extends QueryParamOptions>(
  options: O,
): QueryParams<OptionsForm<O>, string> => {
  const initializer = initialQueryParamState[options.form || "object"];
  return initializer() as QueryParams<OptionsForm<O>, string>;
};
