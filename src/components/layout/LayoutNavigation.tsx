...
/* Both navigation variants render unconditionally and CSS decides which one displays: the sidebar
   rail hides at or below the mobile-navigation cutoff (the `max-[450px]` variant mirrors
   `MobileNavigationCutoff`, the same coupling `LayoutMenuButton` carries), and the slide-out menu
   renders nothing until opened - which only the cutoff-gated hamburger button can do, and
...
export const LayoutNavigation = ({ children, nav }: LayoutNavigationProps) => (
  <>
    <div className='contents max-[450px]:hidden'>
      <Sidebar items={nav} />
    </div>
