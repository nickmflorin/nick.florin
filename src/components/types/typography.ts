...
export type FontSize =
  'inherit' | DiscreteFontSize | QuantitativeSize<'px'> | QuantitativeSize<'rem'>;
...
export type LineHeight =
  'inherit' | DiscreteLineHeight | QuantitativeSize<'px'> | QuantitativeSize<'rem'>;
...
export type TextAlign = 'inherit' | DiscreteTextAlign;
...
export const HorizontalFlexAlignClassNames: Record<HorizontalFlexAlign, `justify-${string}`> = {
...
export type FontWeight = 'inherit' | DiscreteFontWeight;
...
export const TitleFontSizeOrderMap = {
  ...
  xxs: 'h6',
  xxxs: 'h6',
} as const satisfies Record<DiscreteFontSize, TypographyComponent<'title'>>;
...
/**
 * Returns the class name applied for a discrete {@link FontSize}.
 *
 * The class is prefixed with `f-size-` rather than `font-size-` because a `font-size-<size>` class
 * name can conflict with Tailwind class names and confuses the TWMerge package.
 *
 * @see https://github.com/dcastil/tailwind-merge/blob/v2.4.0/docs/limitations.md
 */
const getFontSizeClassName = (fontSize: FontSize | undefined): string =>
  fontSize && DiscreteFontSizes.contains(fontSize) ? `f-size-${fontSize}` : '';
...
export const getTypographyStyle = (props: TypographyCharacteristics): CSSProperties => ({
  fontSize:
...
export type TypographyVisibilityState = 'collapsed' | 'expanded';
