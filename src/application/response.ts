...
export const ServerErrorCodes = enumeratedLiterals(
  ["NOT_AUTHENTICATED", "BAD_REQUEST", "FORBIDDEN"] as const,
  {},
);
export type ServerErrorCode = EnumeratedLiteralsType<typeof ServerErrorCodes>;
...
export class ServerError extends Error {
  ...
  public static BadRequest = (message?: string) =>
    new ServerError({ code: ServerErrorCodes.BAD_REQUEST, message });
  ...
  public static Forbidden = (message?: string) =>
    new ServerError({ code: ServerErrorCodes.FORBIDDEN, message });
...
export class ServerSuccess<T> {
  ...
  public toJson = (): ServerSuccessResponseBody<T> => ({ data: this.data });
  ...
  public static OK = <T>(data: T) =>
    new ServerSuccess<T>({ data, statusCode: SuccessStatusCode.HTTP_200_OK });
}
...
const isErrorConfig = <T>(
  config: ServerResponseConfig<T>,
): config is ServerErrorCode | ServerErrorConfig =>
  typeof config === "string" || (config as ServerErrorConfig).code !== undefined;

type ServerResponseRT<T, C extends ServerResponseConfig<T>> = C extends
  | ServerErrorCode
  | ServerErrorConfig
  ? ServerError
  : ServerSuccess<T>;

export const ServerResponse = <T, C extends ServerResponseConfig<T>>(
  config: C,
): ServerResponseRT<T, C> => {
